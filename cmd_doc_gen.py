...
class HelpExtractor:
    ...
    def __init__(self, command: str = None, output_dir: str = "man_pages", max_depth: int = 2, max_subcommands_per_level: int = 150, help_format: str = "default"):
        ...
        logger.info(f"HelpExtractor initialized, max depth: {max_depth}, max subcommands per level: {max_subcommands_per_level}")
    ...
    def _get_help_command(self, command: str) -> List[str]:
        ...
        logger.info(f"Using '{format_name}' format to get help info for command '{command}': {' '.join(help_command)}")
    ...
    def extract_help(self, command: str) -> Dict[str, Any]:
        ...
        while queue:
            ...
            if not success:
                logger.error(f"Failed to get help information for command '{full_command}'")
            ...
            logger.info(f"Starting to parse help text for command '{full_command}' using LLM")
            parsed_help, raw_response = self.parser.parse_help_text(full_command, help_text)
            
            # Save parsed help information
            current_node.parsed_help = parsed_help
            
            # Generate and save man page for each command
            self._save_man_page(full_command, {"command": full_command, "parsed_help": parsed_help, "raw_help": help_text})
            
            # If depth is not reached, process subcommands
            if depth < self.max_depth:
                # Get subcommand list
                subcommands = parsed_help.get("subcommands", [])
                logger.info(f"Command '{full_command}' has {len(subcommands)} subcommands")
                
                # Limit number of subcommands per level
                for i, subcmd_info in enumerate(subcommands[:self.max_subcommands_per_level]):
                    ...
                    if subcmd_name.startswith('-'):
                        logger.warning(f"Subcommand name starts with '-', might be an option: '{subcmd_name}'")
                        continue
                    ...
                    if self._verify_subcommand_exists(child_command):
                        ...
                        logger.info(f"Adding subcommand to queue: {child_command} (depth: {depth + 1})")
                    else:
                        # Remove invalid subcommand
                        logger.warning(f"Subcommand does not exist, removing: {child_command}")
                        del current_node.children[subcmd_name]
                
                if len(subcommands) > self.max_subcommands_per_level:
                    logger.warning(f"Command '{full_command}' has {len(subcommands)} subcommands, but only processing first {self.max_subcommands_per_level}")
        
        # Print complete command tree after processing
        print("\nCommand tree built:")
        self._print_command_tree(root_node)
        
        logger.info(f"Help information extraction for command '{command}' completed, processed {len(processed_commands)} commands")
    ...
    def _print_command_tree(self, root_node: CommandNode, prefix="", is_last=True, depth=0):
    ...
    def run_command(self, cmd: List[str]) -> Tuple[str, bool]:
        ...
        try:
            ...
            if success:
                logger.info(f"Command executed successfully, output length: {output_length} characters")
            else:
                logger.warning(f"Command execution failed, return code: {result.returncode}, output length: {output_length} characters")
                
            return result.stdout, success
        except subprocess.TimeoutExpired:
            logger.error(f"Command execution timed out: {cmd_str}")
            return f"Error: Command execution timed out after 10 seconds: {cmd_str}", False
    ...
    def _verify_subcommand_exists(self, full_command: str) -> bool:
        ...
        if success:
            ...
            logger.warning(f"Subcommand verification failed, might not exist: {full_command}")
    ...
    def save_outputs(self, command: str, command_info: Dict[str, Any]) -> None:
        ...
        logger.info(f"Saving output files for command '{command}', safe filename: {safe_name}")
    ...
    def _save_raw_help_text(self, command: str, help_text: str, help_command: List[str]) -> None:
...
def main():
    ...
    parser.add_argument(
        "--output-dir", "-o", default="man_pages",
        help="Output directory"
    )
    parser.add_argument(
        "--format", "-f", choices=["man", "json", "all"], default="all",
        help="Output format"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-subcommands", "-m", type=int, default=150,
        help="Maximum subcommands per level (default: 150)"
    )
    parser.add_argument(
        "--help-format", choices=["default", "cliff"], default="default",
